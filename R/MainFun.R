...
#' @param is_zero_miss (required only when \code{is_normalized = FALSE}), a logical variable to determine whether the zero in data represents no value or not.
...
MF_measures <- function(data, q = c(0,1,2), fun_cols = 1:ncol(data),
                        single_eco = TRUE, species_cols = NULL,
  ...
  if (single_eco){
    ...
    spe_data <- cbind(data %>% dplyr::select(species_cols),fun_data) %>% as.data.frame()
    two_idx <- combn(1:nrow(spe_data),2)
    tau <- seq(0,1,0.01)
    fun_data[is.na(fun_data)] <- 0
    transform_D <- sqrt(1-abs(cor(fun_data)))
    output <- lapply(1:ncol(two_idx),function(i){
      d_x <- cbind(x1=t((spe_data %>% dplyr::select(-species_cols))[two_idx[1,i],]),
                   x2=t((spe_data %>% dplyr::select(-species_cols))[two_idx[2,i],]))
